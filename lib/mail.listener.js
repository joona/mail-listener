  ...
  MailListener = (function(_super) {
    ...
    MailListener.prototype.start = function() {
      ...
      return this.imap.connect(function(err) {
        if (err) {
          if(_this.debug) util.log("error connecting to mail server " + err);
          return _this.emit("error", err);
        } else {
          if(_this.debug) util.log("successfully connected to mail server");
          _this.emit("server:connected");
          return _this.imap.openBox("INBOX", false, function(err) {
            if (err) {
              if(_this.debug) util.log("error opening mail box " + err);
              return _this.emit("error", err);
            } else {
              if(_this.debug) util.log("successfully opened mail box");
              return _this.imap.on("mail", function(id) {
                if(_this.debug) util.log("new mail arrived with id " + id);
                _this.emit("mail:arrived", id);
                return _this.imap.search(["UNSEEN"], function(err, searchResults) {
                  var fetch;
                  if (err) {
                    if(_this.debug) util.log("error searching unseen emails " + err);
                    return _this.emit("error", err);
                  } else {
                    if(_this.debug) util.log("found " + searchResults.length + " emails");
